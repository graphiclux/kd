...
<div class="images">
	...
	<?php
		if ( has_post_thumbnail() ) {
                        ...
                        $thumbs = WordPress_MagicScroll_get_prepared_selectors($useWpImages);
                        ...
                        if ($useWpImages) {
                            ...
                            $thumb = WordPress_MagicScroll_get_product_image($img_name,'thumb');
                            $thumb2x = WordPress_MagicScroll_get_product_image($img_name,'thumb2x');
                            
                            WordPress_MagicScroll_get_product_variations(); //call only for onload variation check

                            $img = WordPress_MagicScroll_get_product_image($img_name,'original');
                            $img_result = $plugin->getMainTemplate(compact('img','thumb','thumb2x','id','title','alt','description','additionalDescription','link'));
                        }
                        $img_result = preg_replace('/(<a.*?class=\".*?)\"/is', "$1" . ' lightbox-added"', $img_result);
                        $GLOBALS['magictoolbox']['MagicScroll']['main'] = $img_result;
                        $mainHTML = $GLOBALS['magictoolbox']['MagicScroll']['main'];
                        ...
                        $scroll =  WordPress_MagicScroll_LoadScroll($plugin);
