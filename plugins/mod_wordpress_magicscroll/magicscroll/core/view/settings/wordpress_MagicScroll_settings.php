<?php
    ...
    if(isset($_POST["submit"])) {
        ...
        foreach (WordPressMagicScroll_getParamsProfiles() as $profile => $name) {
            ...
            foreach($_POST as $name => $value) {
                if(preg_match('/magicscrollsettings_'.ucwords($profile).'_(.*)/is',$name,$matches)) {
    ...
    if (!function_exists('magictoolbox_WordPress_MagicScroll_get_wordpress_image_sizes')) {
    ...
    function WordPressMagicScroll_get_description(&$description) {
        $result = '';
    ...
    function WordPressMagicScroll_widthout_img($id) {
        $result = false;
    ...
    function WordPressMagicScroll_get_options_groups ($settings, $profile = 'default', $map, $id, $corePath) {
        $html = '';
        $toolAbr = '';
        $abr = explode(" ", strtolower("Magic Scroll"));

        foreach ($abr as $word) $toolAbr .= $word{0};

        if (!isset($settings[$profile])) return false;
        $settings = $settings[$profile];
        $imgSizes = magictoolbox_WordPress_MagicScroll_get_wordpress_image_sizes();
        ...
        foreach($settings as $name => $s) {
            ...
            $html  .= '<tr>';
            $html  .= '<th width="30%">';
            $html  .= '<label for="magicscrollsettings'.'_'.ucwords($profile).'_'. $name.'">'.$before.'</label>';
            ...
            switch($s["type"]) {
                case "array":
                    ...
                    foreach($s["values"] as $p) {
                        $rButtons[strtolower($p)] = '<label><input type="radio" value="'.$p.'"'. ($s["value"]==$p?"checked=\"checked\"":"").' name="magicscrollsettings'.'_'.ucwords($profile).'_'.$name.'" id="magicscrollsettings'.'_'.ucwords($profile).'_'. $name.$p.'">';
                        $pName = ucwords($p);
                        if(strtolower($p) == "yes") {
                            if (WordPressMagicScroll_widthout_img(strtolower($s['id']))) {
                                $rButtons[strtolower($p)] .= WordPressMagicScroll_get_description($description2);
                            } else {
                    ...
                    break;
                case "num":
                    $html .= '<input  style="width:60px;" type="text" name="magicscrollsettings'.'_'.ucwords($profile).'_'.$name.'" id="magicscrollsettings'.'_'.ucwords($profile).'_'. $name.'" value="'.$s["value"].'" />';
                    break;
                case "text":
                    ...
                    $html .= '<input '.$width.' type="text" name="magicscrollsettings'.'_'.ucwords($profile).'_'.$name.'" id="magicscrollsettings'.'_'.ucwords($profile).'_'. $name.'" value="'.$s["value"].'" />';
                    break;
                case "dropdown":
                    $html .= '<select name="magicscrollsettings'.'_'.ucwords($profile).'_'.$name.'" id=magicscrollsettings'.'_'.ucwords($profile).'_'. $name.'">';
        ...
        foreach ($groups as $name => $group) {
            ...
            if ($name == '0') {
                $name = '';
                $group = preg_replace('/(^.*)(Class\sName)(.*?<span>)(All)(<\/span>.*?<span>)(MagicScroll)(<\/span>.*)/is','$1Apply effect to all image links$3Yes$5No$7',$group);
            }
...
<h1>Magic Scroll settings</h1>
<br/>
<p style="margin-right:20px; float:right; font-size:15px; white-space: nowrap;">
        &nbsp;<a href="<?php echo admin_url().'admin.php?page=WordPressMagicScroll-shortcodes-page'; ?>" target="_self">Shortcodes</a>&nbsp;|
...
<form action="" method="post" id="magicscroll-config-form">
    <div id="tabs">

        <?php 
            foreach (WordPressMagicScroll_getParamsProfiles() as $block_id => $block_name) {
            ?>
            <div id="tab-<?php echo $block_id; ?>">
                <?php echo WordPressMagicScroll_get_options_groups($settings, $block_id, $map, $id, $corePath); ?>
            </div>
        <?php }  ?>
    </div>

    <p id="set-main-settings"><input type="submit" name="submit" class="button-primary" value="Save settings" />&nbsp;<a id="resetLink" style="color:red; margin-left:25px;" href="admin.php?page=WordPressMagicScroll-config-page&reset_settings=true">Reset to defaults</a></p>
</form>

<!-- === onlyForMod start: wordpress -->
<div style="font-size:12px;margin:5px auto;text-align:center;">Learn more about the <a href="http://www.magictoolbox.com/magicscroll_integration/" target="_blank">customisation options<span class="dashicons dashicons-share-alt2" style="text-decoration: none;margin-left:2px;"></span></a></div>
<!-- === onlyForMod end -->