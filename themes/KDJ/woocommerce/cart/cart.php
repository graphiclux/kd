        ...
        <div class="checkout_tbl">
            ...
            foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
                ...
                if ($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_cart_item_visible', true, $cart_item, $cart_item_key)) {
					...
					$product_stone_color = (trim($cart_item['variation']['attribute_pa_stone-color'])) ? strtoupper(trim($cart_item['variation']['attribute_pa_stone-color'])) : false;
                    ...
                    if ($_product->backorders_require_notification() && $_product->is_on_backorder($cart_item['quantity']))
                        echo '<p class="backorder_notification">' . __('Available on backorder', 'woocommerce') . '</p>';
                          ...
                          <?=($product_stone_color) ? '<span class="cart_product_color">STONE COLOR: '.$product_stone_color.'</span>' : '';?>
	                            <?
	...
	<div class="cart_discount_code">
      ...
      <? if ( WC()->cart->coupons_enabled() ) { ?>
	      <div class="coupon">
		      ...
		      <label for="coupon_code"><?php _e( 'Discount Code', 'woocommerce' ); ?>:</label><input type="submit" class="button" name="apply_coupon" value="<?php _e( 'APPLY', 'woocommerce' ); ?>" /><input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php _e( 'Enter Code', 'woocommerce' ); ?>" /><? do_action('woocommerce_cart_coupon'); ?>
